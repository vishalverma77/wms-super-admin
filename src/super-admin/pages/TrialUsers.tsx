...
import { fetchTrialUsersRequest } from '../../store/slices/trialUsersSlice';
...
export function TrialUsers() {
  ...
  const [selectedUser, setSelectedUser] = useState<MappedTrialUser | null>(null);
  ...
  const active = mappedUsers.filter(u => u.status === 'Trial Created' || u.status === 'Active').length;
  const expiring = mappedUsers.filter(u => u.daysLeft > 0 && u.daysLeft <= 7).length;
  ...
  return (
    <>
      <div className="pgh">
        <div className="pgh-l">
          <h1>Trial Users</h1>
          <p>Monitor active trials · Expiry tracking · Conversion pipeline</p>
        </div>
      </div>
      ...
      <div className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        {loading ? (
           <div style={{ padding: 40, textAlign: 'center', color: '#8e9fab' }}>Loading trial users...</div>
        ) : error ? (
           <div style={{ padding: 40, textAlign: 'center', color: '#be123c' }}>{error}</div>
        ) : (
              ...
              <tbody>
                {mappedUsers.map(row => (
                  ...
                  >
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: '#7a7876' }}>{row.email}</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ color: '#7a7876', fontSize: 12 }}>{row.contact}</td>
                    <td><span className="tag t-blue">{row.plan}</span></td>
                    <td style={{ color: '#7a7876' }}>{row.started}</td>
                    <td style={{
                      fontWeight: 600,
                      color: row.daysLeft === 0 ? '#a8a5a0'
                        : row.daysLeft <= 3 ? '#be123c'
                          : '#1a1a1a',
                    }}>
                      {row.expiry}
                    </td>
                    ...
                    <td>
                      <span className={`tag ${['Active', 'Trial Created'].includes(row.status) ? 't-green' :
                          row.status === 'Expiring' ? 't-orange' : 't-gray'
                        }`}>
                        {row.status}
                      </span>
                    </td>
                ...
                {mappedUsers.length === 0 && (
                  <tr>
                    <td colSpan={7} style={{ textAlign: 'center', padding: 40, color: '#8e9fab' }}>
      ...
      {selectedUser && (
        <TrialUserDetailModal user={selectedUser} onClose={() => setSelectedUser(null)} />
      )}
    </>
  );
}
...
function TrialUserDetailModal({ user, onClose }: { user: MappedTrialUser, onClose: () => void }) {
  const timeline = [
  ...
  if (user.daysLeft <= 7 && user.daysLeft > 0) {
    timeline.push({ date: 'Pending', action: 'Expiry reminder queued', by: 'System' });
  }
        ...
        <div style={{ padding: '16px 20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div className="kgrid kg4" style={{ marginBottom: 0 }}>
            <div className="kc kc-w">
              ...
              <div className="kn" style={{ color: user.daysLeft <= 3 ? '#be123c' : '#1a1a1a' }}>
                {user.daysLeft > 0 ? `${user.daysLeft}d` : 'Expired'}
              </div>
            </div>
            <div className="kc kc-b">
              <div className="kl">Trial Started</div>
              <div className="kn" style={{ fontSize: 17 }}>{user.started}</div>
            </div>
            <div className="kc kc-r">
              <div className="kl">Expiry Date</div>
              <div className="kn" style={{ fontSize: 17, color: user.daysLeft <= 3 ? '#be123c' : '#1a1a1a' }}>
                {user.expiry}
              </div>
            </div>
            <div className="kc kc-g">
          ...
          <div className="resp-grid">
              ...
              <div className="card" style={{ padding: '20px' }}>
                ...
                <div className="resp-grid-half-18">
                  {[
                    ...
                    <div key={item.label}>
                      <div style={{ fontSize: 11, color: '#7a7876', textTransform: 'uppercase', marginBottom: 3, letterSpacing: '0.04em' }}>
                        {item.label}
                      </div>
                ...
                {user.notes && (
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #f2f2f2' }}>
                    <div style={{ fontSize: 11, color: '#7a7876', textTransform: 'uppercase', marginBottom: 6, letterSpacing: '0.04em' }}>
                      Notes
                    </div>
                    <div style={{ fontSize: 13, color: '#4a4a4a', lineHeight: 1.6 }}>{user.notes}</div>
                  </div>
                )}
              </div>
            ...
            <div className="card" style={{ padding: '20px' }}>
              <div style={{ fontFamily: '"Outfit",sans-serif', fontWeight: 700, fontSize: 14, marginBottom: 18 }}>
                Activity Timeline
              </div>

              {/* Expiry progress bar */}
              <div style={{ marginBottom: 24, padding: '14px', background: '#f9fbfe', borderRadius: 10, border: '1px solid #f2f2f2' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#7a7876', marginBottom: 8 }}>
                  <span>Trial progress</span>
                  <span style={{ fontWeight: 600, color: 'var(--tx)' }}>{user.daysLeft > 0 ? `${user.daysLeft}d remaining` : 'Expired'}</span>
                </div>
                <div style={{ height: 6, background: '#e6eef2', borderRadius: 999, overflow: 'hidden' }}>
                  <div style={{
                    ...
                    background: user.daysLeft <= 3 ? '#be123c' : user.daysLeft <= 7 ? '#b45309' : 'var(--primary)',
                    borderRadius: 999,
                    transition: 'width 0.4s',
                  }} />
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#a8a5a0', marginTop: 6 }}>
                  ...
                  <div key={idx} style={{ display: 'flex', gap: 14, paddingBottom: 20, position: 'relative' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                      <div style={{
                        ...
                        background: 'var(--primary)', flexShrink: 0, marginTop: 3,
                      }} />
                      {idx < timeline.length - 1 && (
                        <div style={{ width: 2, flex: 1, background: '#f2f2f2', marginTop: 4 }} />
                      )}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, fontSize: 14 }}>{event.action}</div>
                      <div style={{ fontSize: 12, color: '#7a7876', marginTop: 3 }}>
                        {event.date} · {event.by}
                      </div>
                    </div>
                  </div>
